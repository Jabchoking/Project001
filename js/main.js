...
// 팝업창 열기
function popupOpen(){
    let $popupFloor = document.querySelectorAll('#mainfo .inner .parkingPopup .floorSelect ul li');
    let $mapMore = document.querySelectorAll('#mainfo .inner .content10 .parkingScale ul li i');
    ...
    // 클로즈 버튼으로 팝업창 닫기
    let $popupClose = document.querySelector('#mainfo .inner .parkingPopup .popupHeader button');
    if($popupClose){
    ...
    // 팝업 층별 안내
    let $popupImg = document.querySelector('#main .inner .parkingPopup .floorSelect img')
    if($popupImg){
        $popupFloor.forEach((item,idx)=>{
            item.addEventListener('click',e=>{
                ...
                $popupImg.setAttribute('src','images/for_sending/03_0'+popupImgNum+'.jpg');
            })
        })
    }
...
let newscurrent = 2;
let newspx = [0, -330, -660, -990, -1320, -1650, -1980, -2310, -2640, -2970, -3300];
...
let noticeCut = 0;
let noticepx = [0, -280, -560, -840, -1120, -1400, -1680, -1960, -2240, -2520, -2800];
if ($notice_li) {
...
function stopview() { clearInterval(viewbennerTimer); }
function stopnews() { clearInterval(newsbennerTimer); }
function stopnotice() { clearInterval(noticebennerTimer); }
...
let docter_list = [
    //가정의학과
    { doc_Name: '강희철', Area: '피로, 건강증진, 평생건강관리, 약물남용, 다발성복합증상', department: '가정의학과', imgurl: '1-1' },
    { doc_Name: '강희택', Area: '장기 암생존자 관리, 만성피로, 대사증후군, 만성질환 관리, 건강증진, 암환자(통증)', department: '가정의학과', imgurl: '1-2' },
    { doc_Name: '김지혜', Area: '건강 증진 및 질병 예방 비만진료 불가', department: '가정의학과', imgurl: '1-3' },
    { doc_Name: '이지원', Area: '대사증후군, 비만, 영양, 이상지질혈증, 노인의학, 성인병관리, 만성피로, 다발성 복합증상 ,영양대사클리닉', department: '가정의학과', imgurl: '', imgurl: '1-4' },
    //간담췌외과
    { doc_Name: '강창무', Area: '췌장암, 담도암, 췌담도계양성질환, 로봇 및 복강경수술', department: '간담췌외과', imgurl: '2-1' },
    { doc_Name: '김경식', Area: '간암, 간문부암, 담도암, 간담도계양성질환, 복강경수술', department: '간담췌외과', imgurl: '2-2' },
    { doc_Name: '김성현', Area: '췌장암, 간암, 담도암, 간췌담도계양성질환, 로봇 및 복강경수술', department: '간담췌외과', imgurl: '2-3' },
    { doc_Name: '최기홍', Area: '간암, 간이식, 간문부암, 담도암, 간담도계양성질환, 복강경 및 로봇수술', department: '간담췌외과', imgurl: '2-4' },
    { doc_Name: '최진섭', Area: '간, 간암, 간이식, 간담췌외과, 복강경 및 로봇수술', department: '간담췌외과', imgurl: '2-5' },
    { doc_Name: '한대훈', Area: '간암, 간이식, 간문부담관암, 전이암, 복강경/로봇 수술', department: '간담췌외과', imgurl: '2-6' },
    //감염내과
    { doc_Name: '구남수', Area: '감염질환, 발열질환, 의료관련감염, 패혈증, 에이즈', department: '감염내과', imgurl: '3-1' },
    { doc_Name: '김정호', Area: '감염질환, 발열질환, 의료관련감염, 패혈증, 에이즈', department: '감염내과', imgurl: '3-2' },
    { doc_Name: '안진영', Area: '감염질환, 발열질환, 에이즈, 여행자감염, 패혈증, 면역저하자 감염', department: '감염내과', imgurl: '3-3' },
    { doc_Name: '염준섭', Area: '말라리아,뎅기열등열대풍토병, 감염질환, 발열질환, 여행자감염, 폐외결핵, 에이즈, 해외 여행자 예방접종', department: '감염내과', imgurl: '3-4' },
    { doc_Name: '정수진', Area: '감염질환, 발열질환, 에이즈, 여행자감염, 패혈증, 면역저하자 감염 ', department: '감염내과', imgurl: '3-5' },
    { doc_Name: '최준용', Area: '감염질환, 발열질환, 의료관련감염, 패혈증, 에이즈', department: '감염내과', imgurl: '3-6' },
...
function searchDoctors(SearchText) {
    ...
    if (Search_result_list.length !== 0) {
        Search_result_list.forEach(i => {
            ...
            creatediv.innerHTML =
                `<img src="./images/search_doc/${i.imgurl}.jpg" alt="">
    ...
    if (Consent_to_personal_information_checkbox) {
         Consent_to_personal_information_checkbox.addEventListener('change', e => {
            the_right_to_refuse_consent_h3.style.color = "#4067F2"
             ...
             the_right_to_refuse_consent_p.addEventListener('click', e => {
                 window.open('login2_popup.html', "_blank", 'width=350 height=200')
             })
         })
     }
         ...
         let timeid = setInterval(() => {
            ...
            if ( time_num >= 4) {
             ...
             the_registration_process_li_button[ time_num].classList.add('on')
             the_registration_process_li_i[ time_num].classList.add('on')
         }, 3000);
...
let additional_information = document.querySelectorAll('.additional_information');
let additional_num=0;
let timenumber = setInterval(Turning_the_time, 3000);

if (additional_information){
    function Turning_the_time() {
        additional_num++;
      if (additional_num >= additional_information.length) {
        additional_num = 0;
      }
      additional_information.forEach(item => {
